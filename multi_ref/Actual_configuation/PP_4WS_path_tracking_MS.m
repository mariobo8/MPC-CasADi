...
R = zeros(6,6); R(1,1) = 1e2; R(2,2) = 1e2; ...
                R(3,3) = 1e2; R(4,4) = 1e2; ...
                R(5,5) = 5e2; R(6,6) = 0;% weighing matrices (controls)
...
W = zeros(6,6); W(1,1) = 1e5; W(2,2) = 1e5;...
                W(3,3) = 4e4; W(4,4) = 4e4;...
                W(5,5) = 4e9; W(6,6) = 1e1; %weight for rate of change
eps = 1e3;
...
while(fin < 20 && mpciter < sim_tim / T )
    ...
    [t0, x0, xp0, u0, up0, ref] = PP_4ws_shift(mpciter, T, t0, x0,...
        u, f, arc_length, s_0, x_p, y_p, N, x_s, x0(1),x0(2),s_00, n_con);
    x_r(:,:,mpciter+1) = ref;
    xx(:,mpciter+2) = x0;
